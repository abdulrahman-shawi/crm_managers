...
export const Sidebar = ({ isDesktopCollapsed, isMobileOpen, onDesktopToggle, onMobileClose }: SidebarProps) => {
  ...
  return (
    <aside className={`
        ...
        md:translate-x-0 ${isDesktopCollapsed ? "md:w-[88px]" : "md:w-[280px]"}
      `}>
        ...
        <div className="h-20 flex items-center px-6 mb-4 border-b border-slate-100 dark:border-slate-900">
          <div className="flex items-center gap-3 min-w-max">
            ...
            <div className={`transition-all duration-300 ${isDesktopCollapsed ? "md:opacity-0 md:translate-x-4" : "opacity-100"}`}>
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden px-4 space-y-8 custom-scrollbar">
          {menuGroups.map((group, idx) => (
            <div key={idx} className="space-y-2">
              <p className={`px-4 text-[11px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-[2px] transition-opacity duration-300 ${isDesktopCollapsed ? "md:opacity-0" : "opacity-100"}`}>
                {group.group}
              </p>
                  ...
                  return (
                    ...
                    >
                      <item.icon size={22} className={`shrink-0 ${isActive ? "animate-pulse" : "group-hover:scale-110 transition-transform"}`} />
                                            ...
                                            <span className={`font-bold text-sm whitespace-nowrap transition-all duration-300 ${isDesktopCollapsed ? "md:opacity-0 md:translate-x-10" : "opacity-100"}`}>
                        {item.label}
                      </span>
        ...
        <div className="p-4 mt-auto">
          <div className={`p-3 rounded-2xl bg-slate-100 dark:bg-slate-900/50 border border-slate-200/50 dark:border-slate-800 transition-all ${isDesktopCollapsed ? "md:p-2" : "p-3"}`}>
            <div className="flex items-center gap-3">
              ...
              <div className={`transition-all duration-300 ${isDesktopCollapsed ? "md:hidden" : "block"}`}>
                <p className="text-xs font-black text-slate-800 dark:text-white truncate">عبدالرحمن الشاوي</p>
                <p className="text-[10px] text-slate-500 font-medium truncate">{user?.email}</p>
              </div>
            </div>
            
            <button className={`mt-3 w-full flex items-center justify-center gap-2 h-10 rounded-xl text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors ${isDesktopCollapsed ? "md:h-10 md:w-10 md:mx-auto md:p-0" : "px-3"}`}>
              <LogOut size={18} />
              {!isDesktopCollapsed && <span className="font-bold text-xs text-left w-full">خروج</span>}
            </button>
          </div>
        </div>
      </aside>
