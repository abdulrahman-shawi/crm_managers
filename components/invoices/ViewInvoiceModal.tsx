        ...
        <AnimatePresence>
            <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md overflow-y-auto" onClick={onClose}>
                <motion.div
                    ...
                    className="bg-white text-slate-900 w-full max-w-4xl h-[600px] overflow-y-scroll rounded-3xl shadow-2xl overflow-hidden my-8"
                    dir="rtl"
                    onClick={(e) => e.stopPropagation()}
                >
                    <div className="bg-slate-50 p-4 flex justify-between items-center border-b no-print">
                        <button onClick={() => window.print()} className="bg-blue-600 text-white px-6 py-2 rounded-xl flex items-center gap-2 font-bold">
                    ...
                    <div className="p-12 bg-white" id="printable-area">
                        <div className="flex justify-between items-start mb-12 border-b-2 border-slate-100 pb-10">
                            <div>
                                <h1 className="text-4xl font-black text-blue-600 mb-2 italic">
                            ...
                            <div className="text-left font-black text-2xl">قطنيات السلطان</div>
                        </div>
                        ...
                        <div className="mb-12 bg-slate-50 p-8 rounded-[2.5rem] flex justify-between items-center">
                            ...
                            <div className={`px-5 py-2 rounded-full text-xs font-black text-white ${invoice.status === 'مدفوعة' ? 'bg-emerald-500' : 'bg-amber-500'}`}>
                        ...
                        <table className="w-full mb-10 text-right">
                            <thead>
                                <tr className="bg-slate-900 text-white">
                                    <th className="px-6 py-4 rounded-r-2xl">المنتج</th>
                            ...
                            <tbody>
                                {invoice.rawItems?.map((item: any, idx: number) => (
                                    <tr key={idx} className="border-b border-slate-100">
                                        <td className="px-6 py-6 font-bold text-slate-700">{getProductName(item.productId)}</td>
                        ...
                        <div className="flex justify-end">
                            <div className="w-80 bg-slate-50 p-6 rounded-[2.5rem] border border-slate-100">
                                <div className="flex justify-between text-2xl font-black text-blue-600">
                                    <span>الإجمالي:</span>
                                    <span className="italic">ل.س{subtotal.toLocaleString()}</span>
                                </div>
                            </div>
                        </div>
                    </div>
