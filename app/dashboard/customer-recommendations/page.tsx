...
export default function CustomerRecommendationsPage() {
  ...
  // جلب العملاء لاستخدامهم في القائمة المنسدلة
  ...
  return (
    <div className="space-y-8" dir="rtl">
      ...
      <AddTestimonialModal
        ...
        onAdd={(newT: Testimonial) => setTestimonials([newT, ...testimonials])}
        customers={customers}
      />
    </div>
  );
...
// مكون المودال
function AddTestimonialModal({ isOpen, onClose, onAdd, customers }: any) {
  ...
  const handleSubmit = async () => {
    if (!formData.name || !formData.text) return alert("يرجى اختيار العميل وكتابة التوصية");
